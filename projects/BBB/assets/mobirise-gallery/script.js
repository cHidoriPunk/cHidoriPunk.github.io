(function($) {
    ...
    $(document).on('add.cards change.cards', function(event) {
        ...
        $section.find('.mbr-gallery-item').each(function(el) {
            var tagImgAttr = ($(this).attr('data-imageurl')||"").trim();
            var tagsAttr = ($(this).attr('data-tags')||"").trim();
            var tagImgList = tagImgAttr.split(',');
            ...
            tagImgList.map(function(el) {
                var tagImg = el.trim();
                if ($.inArray(tagImg, filterImageList) == -1)
                    filterImageList.push(tagImg);
            })
        })
        if ($section.find('.mbr-gallery-filter').length > 0 && $(event.target).find('.mbr-gallery-filter').hasClass('gallery-filter-active')) {
            var filterHtml = '<ul><li class="mbr-gallery-filter-all active"><img src="assets/images/filter-images/bbb_01.png" title="הצג הכל" alt="select all"/><span>הצג הכל</span></li>';

            $section.find('.mbr-gallery-filter').html('');
            filterList.map(function(el, i) {
                filterHtml += '<li><img class="img-responsive" src="'+ filterImageList[i] +'" title="'+ el +'"/><span>'+ el +'</span></li>'
            })
            filterHtml += '</ul>';
            ...
            $section.on('click', '.mbr-gallery-filter li', function(e) {
                ...
                var filter = $li.find('span').html().trim();
